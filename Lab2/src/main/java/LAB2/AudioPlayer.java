...
public class AudioPlayer {
    ...
    /**
     * Починає відтворення поточного треку, якщо плеєр не грає, плейлист не порожній та батарея заряджена.
     * @throws IOException якщо виникає помилка при запису в лог
     */
    public void play() throws IOException {
        if (!isPlaying && !playList.isEmpty() && battery.getCharge() > 0) {
            ...
            logger.log(String.format("Початок відтворення: %s", playList.get(currentTrackIndex).getTitle()));
            System.out.printf("Початок відтворення: %s\n", playList.get(currentTrackIndex).getTitle());
        } else {
    ...
    /**
     * Зупиняє відтворення поточного треку, якщо плеєр грає.
     * @throws IOException якщо виникає помилка при запису в лог
     */
    public void stop() throws IOException {
        if (isPlaying) {
            ...
            logger.log("Зупинка відтворення");
